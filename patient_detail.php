...
// ─── Vérification : praticien OU admin connecté ──────────────────────────
$isAdminConnected = !empty($_SESSION['apa4cad_admin_authenticated'] ?? false);
$fromAdmin = !empty($_GET['from_admin'] ?? '') && $isAdminConnected;

if (!isPraticienLoggedIn() && !$isAdminConnected) {
...
<?php if ($fromAdmin): ?>
<!-- Bannière admin -->
<div style="background:linear-gradient(90deg,#dc2626,#ef4444);color:#fff;padding:9px 20px;
            font-size:12px;font-weight:600;display:flex;align-items:center;justify-content:center;
            gap:14px;flex-wrap:wrap;position:sticky;top:0;z-index:1000;
            box-shadow:0 2px 8px rgba(220,38,38,.25)">
    <span style="display:flex;align-items:center;gap:8px">
        🛠 <strong>Mode administrateur</strong> · consultation en lecture
    </span>
    <a href="admin/patients_all.php" style="background:rgba(255,255,255,.18);color:#fff;
       text-decoration:none;padding:4px 12px;border-radius:6px;font-size:11px;
       border:1px solid rgba(255,255,255,.3)">
        ← Tous les patients
    </a>
    <a href="admin/index.php" style="background:rgba(255,255,255,.18);color:#fff;
       text-decoration:none;padding:4px 12px;border-radius:6px;font-size:11px;
       border:1px solid rgba(255,255,255,.3)">
        🏠 Dashboard admin
    </a>
...
<div class="app">
    ...
    <section class="banner">
        <div class="crumbs">
            <?php if ($fromAdmin): ?>
                <a href="admin/patients_all.php">Tous les patients</a><span class="sep">›</span><span><?= h($patientName) ?></span>
            <?php else: ?>
                <a href="patient.php">Mes patients</a><span class="sep">›</span><span><?= h($patientName) ?></span>
            <?php endif; ?>
        </div>
        <h1><?= h($patientName) ?></h1>
        <div class="meta">
            ...
            <div class="presc-list">
                <?php foreach ($prescriptions as $pr): ?>
                    <div class="presc-row">
                        ...
                        <a href="prescription_detail.php?id=<?= urlencode($pr['fragment']) ?><?= $fromAdmin ? '&from_admin=1' : '' ?>" class="view-btn">Voir →</a>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php endif; ?>
