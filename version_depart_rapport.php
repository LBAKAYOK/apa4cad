...
function loadModalitiesPerActivity(array $activityLocalNames): array
{
    if (empty($activityLocalNames)) return [];
    $values = implode("\n    ", array_map(fn($n) => 'ex:' . $n, $activityLocalNames));
    $query = '
...
$modalitiesPerActivity = !empty($finalRecos)
    ? loadModalitiesPerActivity(array_map(fn($r) => $r['activity'], $finalRecos))
    : [];
...
// URL retour
$backUrl = 'index.php?' . http_build_query(['pathologies' => $selected]);
...
foreach ($finalRecos as $item) {
    if (!empty($item['activity'])) {
        $finalActivityUris[] = NS . $item['activity'];
    }
}
setSessionActivities($finalActivityUris);

// ── Module 2 : stocker les contre-indications BLOQUANTES (avec raison) ────
// blockedRecos contient les activités candidates retirées à cause d'une CI
$contraindications = [];
foreach ($blockedRecos as $item) {
    $activityLabel = prettyLabel($item['activity']);
    // Extraire les pathologies qui ont bloqué (déduplique)
    $pathos = [];
    foreach ($item['blockReasons'] ?? [] as $reason) {
        if (!empty($reason['patho']) && !in_array($reason['patho'], $pathos, true)) {
            $pathos[] = $reason['patho'];
        }
    }
    $contraindications[] = [
